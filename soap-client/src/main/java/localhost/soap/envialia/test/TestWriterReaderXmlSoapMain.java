...
import localhost.soap.envialia.estado.pojo.RConsultaPojo;
import localhost.soap.envialia.estado.pojo.REnvioEstadoPojo;
...
public class TestWriterReaderXmlSoapMain {
	...
	@SuppressWarnings("unchecked")
	public void jaxbMarshalPojoToXmlAgainForConsulta () {
		...
		REnvioEstadoPojo rEEP2 = new REnvioEstadoPojo();
		rEEP2.setvCodTipoEst("02");
		rEEP2.setdFecHoraAlta("10/29/2023");

		List<REnvioEstadoPojo> rEEPList = new ArrayList<>();
		rEEPList.add(rEEP1);
		rEEPList.add(rEEP2);

		RConsultaPojo rCP = new RConsultaPojo();
		rCP.setEstados(rEEPList);
		...
		try {
			jaxbContext = JAXBContext.newInstance(rCP.getClass());
	...
	@SuppressWarnings("unchecked")
	public void jaxbUnmarshalPojoToXmlAgainForConsulta () {

		String xmlStr = "<CONSULTA><ENV_ESTADOS V_COD_TIPO_EST=\"01\" D_FEC_HORA_ALTA=\"09/28/2022\"/><ENV_ESTADOS V_COD_TIPO_EST=\"02\" D_FEC_HORA_ALTA=\"10/29/2023\"/></CONSULTA>";
		...
		try {
			...
			rCP = (RConsultaPojo) unmarshaller.unmarshal(source);

			json = objectMapper.writeValueAsString(rCP);
